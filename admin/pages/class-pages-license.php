...
class License extends \Wbcr_FactoryClearfy000_LicensePage {
	...
	public function get_plan_description()
	{
		...
		$description = '<p style="font-size: 16px;">' . __('<b>Titan Security PRO</b> is a paid package of components for the popular free WordPress plugin named Titan Security PRO. You get access to all paid components at one price.', 'clearfy') . '</p>';
		...
		if( !$this->plugin->premium->is_activate() ) {
			$description .= '<p>The free trial edition (no credit card) contains all of the features included in the paid-for version
                    of the product.</p>';
			$description .= '<a href="" data-url="' . esc_url($activate_trial_url) . '" id="js-wtitan-activate-trial-button" class="button button-default">' . __('Activate 30 days trial', 'anti-spam') . '</a>';
		}
