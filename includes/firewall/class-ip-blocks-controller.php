	...
	class wfWAFIPBlocksController {
		...
		public static function synchronizeConfigSettings()
		{
			...
			foreach($blocks as $b) {
				$patternBlocks[] = array(
					'id' => $b->id,
					'ipRange' => $b->ipRange,
					'hostnamePattern' => $b->hostname,
					'uaPattern' => $b->userAgent,
					'refPattern' => $b->referrer,
		...
		public function shouldBlockRequest($request)
		{
			...
			if( is_array($otherBlocks) ) {
				...
				foreach($blocks as $b) {
					...
					return array(
						'action' => (empty($b['reason']) ? '' : $b['reason']),
						'id' => $b['id'],
						'block' => true
					);
				}
			...
			if( is_array($lockouts) ) {
				...
				if( $isAuthRequest ) {
					foreach($lockouts as $l) {
						...
						return array(
							'action' => (empty($l['reason']) ? '' : $l['reason']),
							'id' => $l['id'],
							'lockout' => true
						);
					}
		...
		protected function checkForWhitelisted($ip)
		{
			...
			if( is_array($serverIPs) ) {
				if( (isset($_SERVER['SCRIPT_FILENAME']) && $_SERVER['SCRIPT_FILENAME'] == realpath($pluginABSPATH . DIRECTORY_SEPARATOR . 'wp-cron.php')) || //Safe -- plugin will do a final check to make sure the cron constant is defined
		...
		protected function ip2Country($ip)
		{
			...
			require_once(dirname(__FILE__) . '/class-firewall-geo-ip2.php');
