...
class wfLog {
	...
	/**
	 * @var \WBCR\Titan\Firewall\Model\Request
	 */
	...
	public static function isHumanRequest($IP = false, $UA = false)
	{
		...
		if( $wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM {$table} WHERE IP = %s AND identifier = %s AND expiration >= UNIX_TIMESTAMP()", \WBCR\Titan\Firewall\Utils::inet_pton($IP), hash('sha256', $UA, true))) ) {
			return true;
		}
	...
	public function __construct($apiKey, $wp_version)
	{
		...
		$this->hitsTable = WBCR\Titan\Database\Schema::get_table_name('hits');
		$this->loginsTable = WBCR\Titan\Database\Schema::get_table_name('logins');
		$this->blocksTable = \WBCR\Titan\Firewall\Model\Block::blocksTable();
	...
	public function initLogRequest()
	{
		if( $this->currentRequest === null ) {
			$this->currentRequest = new \WBCR\Titan\Firewall\Model\Request();
	...
	/**
	 * @return \WBCR\Titan\Firewall\Model\Request
	 */
	...
	{ //404 or hit
		if( !\WBCR\Titan\Firewall\Model\Rate_Limit::mightRateLimit($type) ) {
			return;
		}
	...
	public function processGetHitsResults($type, &$results)
	{
		...
		$patternBlocks = \WBCR\Titan\Firewall\Model\Block::patternBlocks(true);
		...
		foreach($results as &$res) {
			...
			$ipBlock = \WBCR\Titan\Firewall\Model\Block::findIPBlock($res['IP']);
			if( $ipBlock !== false ) {
				$res['blocked'] = true;
	...
	public function firewallBadIPs()
	{
		...
		foreach($patternBlocks as $b) {
			if( $b->matchRequest($IP, $userAgent, $referrer) !== \WBCR\Titan\Firewall\Model\Block::MATCH_NONE ) {
				$b->recordBlock();
		...
		// Country blocking
		$countryBlocks = \WBCR\Titan\Firewall\Model\Block::countryBlocks(true);
		foreach($countryBlocks as $b) {
			$match = $b->matchRequest($IP, false, false);
			if( $match === \WBCR\Titan\Firewall\Model\Block::MATCH_COUNTRY_REDIR_BYPASS ) {
		...
		//Specific IP blocks
		$ipBlock = \WBCR\Titan\Firewall\Model\Block::findIPBlock($IP);
		if( $ipBlock !== false ) {
			$ipBlock->recordBlock();
			...
			if( $ipBlock->type == \WBCR\Titan\Firewall\Model\Block::TYPE_IP_MANUAL || $ipBlock->type == \WBCR\Titan\Firewall\Model\Block::TYPE_IP_AUTOMATIC_PERMANENT ) {
	...
	private function takeBlockingAction($configVar, $reason)
	{
		if( $this->googleSafetyCheckOK() ) {
			...
			if( $action == 'block' ) { //Rate limited - block temporarily
				...
				\WBCR\Titan\Firewall\Model\Block::createRateBlock($reason, $IP, $secsToGo);
				wfActivityReport::logBlockedIP($IP, null, 'throttle');
				$this->tagRequestForBlock($reason);

				$alertCallback = array(new \WBCR\Titan\Firewall\Model\BlockAlert($IP, $reason, $secsToGo), 'send');
				...
				$secsToGo = \WBCR\Titan\Firewall\Model\Block::rateLimitThrottleDuration();
				\WBCR\Titan\Firewall\Model\Block::createRateThrottle($reason, $IP, $secsToGo);
				wfActivityReport::logBlockedIP($IP, null, 'throttle');
